...
function MetricsContent() {
  ...
  const [selectedMetric, setSelectedMetric] = useState<'cls' | 'lcp' | 'inp'>(
    'cls'
  )
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/metrics?urlId=${selectedUrlId}&period=${period}`
      )
      const data = await response.json()
      setMetrics(data)
    } catch (_) {
  ...
  const filteredUrls = urls
    ...
    .sort((a, b) => {
      ...
      const aExactMatch =
        a.url.toLowerCase() === debouncedSearchTerm.toLowerCase()
      const bExactMatch =
        b.url.toLowerCase() === debouncedSearchTerm.toLowerCase()
                ...
                <div className='p-6 rounded-lg border'>
                  ...
                  <div className='space-y-2'>
                    <p>
                      En Düşük:{' '}
                      <span
                        className={`font-medium ${getMetricStatus(minMaxMetrics?.cls.min || 0, 'cls') === 'good' ? 'text-green-600' : getMetricStatus(minMaxMetrics?.cls.min || 0, 'cls') === 'needs-improvement' ? 'text-yellow-600' : 'text-red-600'}`}
                      >
                        {minMaxMetrics?.cls.min.toFixed(3)}
                      </span>
                    </p>
                    <p>
                      En Yüksek:{' '}
                      <span
                        className={`font-medium ${getMetricStatus(minMaxMetrics?.cls.max || 0, 'cls') === 'good' ? 'text-green-600' : getMetricStatus(minMaxMetrics?.cls.max || 0, 'cls') === 'needs-improvement' ? 'text-yellow-600' : 'text-red-600'}`}
                      >
                        {minMaxMetrics?.cls.max.toFixed(3)}
                      </span>
                    </p>
                  </div>
                </div>
                ...
                <div className='p-6 rounded-lg border'>
                  ...
                  <div className='space-y-2'>
                    <p>
                      En Düşük:{' '}
                      <span
                        className={`font-medium ${getMetricStatus(minMaxMetrics?.lcp.min || 0, 'lcp') === 'good' ? 'text-green-600' : getMetricStatus(minMaxMetrics?.lcp.min || 0, 'lcp') === 'needs-improvement' ? 'text-yellow-600' : 'text-red-600'}`}
                      >
                        {minMaxMetrics?.lcp.min.toFixed(0)}ms
                      </span>
                    </p>
                    <p>
                      En Yüksek:{' '}
                      <span
                        className={`font-medium ${getMetricStatus(minMaxMetrics?.lcp.max || 0, 'lcp') === 'good' ? 'text-green-600' : getMetricStatus(minMaxMetrics?.lcp.max || 0, 'lcp') === 'needs-improvement' ? 'text-yellow-600' : 'text-red-600'}`}
                      >
                        {minMaxMetrics?.lcp.max.toFixed(0)}ms
                      </span>
                    </p>
                  </div>
                </div>
                ...
                <div className='p-6 rounded-lg border'>
                  ...
                  <div className='space-y-2'>
                    <p>
                      En Düşük:{' '}
                      <span
                        className={`font-medium ${getMetricStatus(minMaxMetrics?.inp.min || 0, 'inp') === 'good' ? 'text-green-600' : getMetricStatus(minMaxMetrics?.inp.min || 0, 'inp') === 'needs-improvement' ? 'text-yellow-600' : 'text-red-600'}`}
                      >
                        {minMaxMetrics?.inp.min.toFixed(0)}ms
                      </span>
                    </p>
                    <p>
                      En Yüksek:{' '}
                      <span
                        className={`font-medium ${getMetricStatus(minMaxMetrics?.inp.max || 0, 'inp') === 'good' ? 'text-green-600' : getMetricStatus(minMaxMetrics?.inp.max || 0, 'inp') === 'needs-improvement' ? 'text-yellow-600' : 'text-red-600'}`}
                      >
                        {minMaxMetrics?.inp.max.toFixed(0)}ms
                      </span>
                    </p>
                  </div>
                </div>
